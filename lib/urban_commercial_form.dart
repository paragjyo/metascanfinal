...
class UrbanCommercial extends StatefulWidget {
  final String title;

  const UrbanCommercial({Key? key, required this.title}) : super(key: key);
  @override
  State<UrbanCommercial> createState() => _UrbanCommercialState();
}

class _UrbanCommercialState extends State<UrbanCommercial> {
  String parag = "";
  ...
  String _municipal_board = "";

  String _road_name = "";

  String _subway_name = "";

  String _bye_lane_name = "";
  ...
  //Image Resize and upload variables
  ...
  Future<String> loadJson() async {
    String jsonString = await rootBundle.loadString('assets/district-municipal.json');
    setState(() {
      globalJson = json.decode(jsonString);
      _districts = globalJson.keys.toList();
      _selectedDistrict = _districts[0];
      _municipal = List<String>.from(globalJson[_selectedDistrict].values);
      _selectedBlock = _municipal[0];
    });

    return parag;
  }

  // IMAGE UPLOAD and RESIZE FUNCTIONS
...
// IMAGE UPLOAD and RESIZE FUNCTIONS
  ...
  Future<void> _syncData() async {
    ...
    List<Map> data = await database.rawQuery('SELECT * FROM survey_form_urban_test');
    ...
    for (var item = 0; item < data.length; item++) {
      ...
      //     'SELECT image FROM survey_form_urban_test WHERE id = ${idTaken}');
      // print(result);
...
// IP ADDRESS
  ...
  Future<String> getDatabasePath() async {
    ...
    final databasePath = join(directory.path, 'test2.db');
    ...
    final database = await openDatabase(
      ...
      onCreate: (db, version) {
        return db.transaction((txn) async {
          ...
          await txn.execute('''
   CREATE TABLE IF NOT EXISTS survey_form_urban_test (id INTEGER PRIMARY KEY ,
district TEXT DEFAULT NULL,
block_development_office TEXT DEFAULT NULL,
road_name TEXT DEFAULT NULL,
subway_name TEXT DEFAULT NULL,
bye_lane_name TEXT DEFAULT NULL,
...
// SAVE THE FULL FORM DATA
  void _saveFullUserDataToDatabase() async {
    ...
    await database.insert(
      'survey_form_urban_test',
      {
        "district": _district,
        "block_development_office": _municipal_board,
        "road_name": _road_name,
        "subway_name": _subway_name,
        "bye_lane_name": _bye_lane_name,
        "ward_no": _ward_no,
...
// IMAGE UPLOAD WITH CAMERA AND FRM GALLERY
  ...
  Future<void> _saveImageLocal(ImageSource imagePickMode) async {
    final image = await ImagePicker().pickImage(source: imagePickMode);
    if (image != null) {
      setState(() {});
    }
    // Database _databaseImage = await openDatabase(
    ...
    if (image != null) {
      // Use the pickedFile instance to work with the selected image
      // For example, you can get the file path using pickedFile.path
      print('Selected image path: ${image.path}');

      // Use flutter_exif_rotation to fix the image orientation
      File fixedImage = await FlutterExifRotation.rotateImage(path: image.path);

      // Use flutter_native_image to get additional information about the image
      ImageProperties properties = await FlutterNativeImage.getImageProperties(fixedImage.path);
      print('Image resolution: ${properties.width}x${properties.height}');
      print('Image creation date: ${properties.orientation}');
    }

    if (image == null) return;

    _image = image.path;
    // await _databaseImage.insert(
    //   'images',
    //   {'path': image.path},
    //   conflictAlgorithm: ConflictAlgorithm.replace,
    // );
    ...
    // setState(() {});
    // List<Map> dataImage = await _databaseImage.rawQuery('SELECT * FROM images');
    // print(dataImage);
    // await _databaseImage.close();
  }
                                  ...
                                  children: [
                                    ...
                                    DropdownButtonFormField<String>(
                                      ...
                                      onChanged: (value) {
                                        ...
                                        _municipal_board = _selectedBlock;
                                      },
                                    ),
                                    SizedBox(
                                      height: 15,
                                    ...
                                    TextFormField(
                                      ...
                                      onChanged: (value) => _bye_lane_name = value as String,
                                    ),
                                    SizedBox(
                                      height: 15,
                                    ),
                                    ...
                                    TextFormField(
                                      ...
                                      onChanged: (value) => _subway_name = value as String,
                                    ),
                                    SizedBox(
                                      height: 15,
                                    ),
                                ...
                                children: [
                                  ...
                                  TextFormField(
                                      decoration: InputDecoration(
                                        ...
                                        fillColor: Colors.grey[200],
                                        border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none),
                                        labelText: 'Landowner of the shop',
                                        hintStyle: TextStyle(color: Colors.grey[500]),
                                      ),
                                      validator: (value) {
                                        if (value?.isEmpty ?? true) {
                                      ...
                                      onChanged: (value) {
                                        _landowner_of_the_shop = value as String;
                                        print(_landowner_of_the_shop);
                                      }),
                                  SizedBox(
                                    height: 15,
                                  ),
                                  TextFormField(
                                      decoration: InputDecoration(
                                        ...
                                        fillColor: Colors.grey[200],
                                        border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none),
                                        labelText: 'Father\'s / Husband\'s Name of Land Owner',
                                        hintStyle: TextStyle(color: Colors.grey[500]),
                                      ),
                                      validator: (value) {
                                        if (value?.isEmpty ?? true) {
                                      ...
                                      onChanged: (value) {
                                        _father_name_of_the_landowner = value as String;
                                      }),
                                  SizedBox(
                                    height: 15,
                                  ),
                                  ...
                                  TextFormField(
                                      ...
                                      onChanged: (value) {
                                        _trade_license_no = value as String;
                                      }),
                                  SizedBox(
                                    height: 15,
                                  ),
                                  TextFormField(
                                      ...
                                      onChanged: (value) {
                                        _name_of_the_shop_owner = value as String;
                                      }),
                                  SizedBox(
                                    height: 15,
                                  ),
                                  TextFormField(
                                      ...
                                      decoration: InputDecoration(
                                        ...
                                        fillColor: Colors.grey[200],
                                        border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none),
                                        labelText: 'Father/Husband\'s name of the shop owner',
                                        hintStyle: TextStyle(color: Colors.grey[500]),
                                      ),
                                      validator: (value) {
                                        if (value?.isEmpty ?? true) {
                                      ...
                                      onChanged: (value) {
                                        _father_name_of_the_shop_owner = value as String;
                                      }),
                                  SizedBox(
                                    height: 15,
                                  ),
                                  ...
                                  TextFormField(
                                      ...
                                      onChanged: (value) {
                                        _shop_office_no = value as String;
                                      }),
                                  SizedBox(
                                    height: 15,
                                  ),
                                  DropdownButtonFormField<String>(
                                    ...
                                    onChanged: (value) {
                                      setState(() {
                                        _type_structure_of_shop = value!;
                                      });
                                    },
                                    hint: Text('Structure of shop'),
                                  ),
                                  SizedBox(
                                    height: 15,
                                  ),
                                  ...
                                  DropdownButtonFormField<String>(
                                    ...
                                    onChanged: (value) {
                                      setState(() {
                                        _electricity_facilities = value!;
                                      });
                                    },
                                    hint: Text('Electricity facilities'),
                                  ),
                                  SizedBox(
                                    height: 10,
                                  ),
