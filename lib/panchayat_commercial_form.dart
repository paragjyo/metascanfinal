...
class PanchayatCommercial extends StatefulWidget {
  final String title;

  const PanchayatCommercial({Key? key, required this.title}) : super(key: key);
  @override
  State<PanchayatCommercial> createState() => _PanchayatCommercialState();
}

class _PanchayatCommercialState extends State<PanchayatCommercial> {
  String parag = "";
  ...
  Future<List<Map<String, dynamic>>> _query() async {
    final database = await openDatabase(
      path.join(await getDatabasesPath(), 'test1.db'),
      version: 1,
    );
...
// IP ADDRESS
  ...
  Future<String> getDatabasePath() async {
    ...
    final databasePath = path.join(directory.path, 'test1.db');
...
// IMAGE UPLOAD and RESIZE FUNCTIONS
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text("Panchayat Commercial"),
      ),
      body: Container(
